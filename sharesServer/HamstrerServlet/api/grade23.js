...
       responseType:'arraybuffer'
    ...
    function cb (flag) {
        ...
        consoles.log('fileArr ->', fileArr.length);
        let arr = fileArr.map(item => item.codeID);
        ...
        consoles.log('testData ->', testData, testCurr, MaxNumber.length);
        if (testData) {
    ...
    if (k__link.length > 2) {
        ...
        if (name > 0) {
            ...
            MaxNumber[name].push({
                code: code,
                nub: score.numner,
                max: bfb(k_link[testCurr - 1].max, k_link[testCurr].js),
                min: bfb(k_link[testCurr - 1].min, k_link[testCurr].js),
                upNub: bfb(k_link[testCurr - 1].js, k_link[testCurr].js),
...
function bfb (a, b) {
    return parseInt((a - b) / b * 10000) / 100
}