...
module.exports = function ($) {
  ...
  async function getApi (index, len) {
    ...
    if (item['K-Lin']) {
      ...
      // 复权
      (/^s[z|h](600|601|603|605|900|000|002|200|300)/.test(code)) && fliter(k_link)
      k_link[0].boll = boll(item['K-Lin'], k_link[0])
...
function boll (k_link, o) {
  ...
  let MA = 0; let MD = 0; let MB = 0; let UP = 0; let DN = 0; let mean = 0; let sum = 0; let arr = []; let i1 = 0; let k1 = 0; let k2 = 0
...
function ENE (list) {
  ...
  var MA = list.slice(0, 10)

  var UPPER = (1 + M1 / 100) * MA.sum('js')

  var LOWER = (1 - M2 / 100) * MA.sum('js')

  var ENE = (UPPER + LOWER) / 2
  let obj = {
    UPPER,
    LOWER,
    ENE
...
// 前复权
function fliter (k_link) {
  let qfq = null
  if (k_link[0] && k_link[1]) {
    let up = (k_link[0].ks - k_link[1].js) / k_link[1].js
    if (up > 0.11 || up < -0.11) {
      qfq = k_link[0].ks / k_link[1].js
    }
  }
  if (qfq) {
    for (let i = 1; i < k_link.length; i++) {
      let item = k_link[i]
      item['max'] = item['max'] * qfq
      item['min'] = item['min'] * qfq
      item['mean'] = item['mean'] * qfq
      item['ks'] = item['ks'] * qfq
      item['js'] = item['js'] * qfq
      item['volume'] = item['volume'] * qfq
      item['mean5'] = item['mean5'] * qfq
      item['mean10'] = item['mean10'] * qfq
      item['mean20'] = item['mean20'] * qfq
      item['mean30'] = item['mean30'] * qfq
      item['status'] = item['status'] * qfq
      item['boll']['MA'] = item['boll']['MA'] * qfq
      item['boll']['MD'] = item['boll']['MD'] * qfq
      item['boll']['MB'] = item['boll']['MB'] * qfq
      item['boll']['UP'] = item['boll']['UP'] * qfq
      item['boll']['DN'] = item['boll']['DN'] * qfq
      item['MACD']['EMA_12'] = item['MACD']['EMA_12'] * qfq
      item['MACD']['EMA_26'] = item['MACD']['EMA_26'] * qfq
      item['MACD']['EMA_DIF'] = item['MACD']['EMA_DIF'] * qfq
      item['MACD']['EMA_DEA'] = item['MACD']['EMA_DEA'] * qfq
      item['MACD']['EMA_BAR'] = item['MACD']['EMA_BAR'] * qfq
    }
  }
}
